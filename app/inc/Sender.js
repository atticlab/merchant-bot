...
var Sender = (order) => {
    ...
    Helpers.getObjectByBucketAndID(Conf.riak_options.order.bucket_name, order.id)
        .then(function(riak_object){
            var order_data = Helpers.decodeRiakData(riak_object.value);
            if (order_data.status != Conf.statuses.STATUS_WAIT_ANSWER) {
                return false;
            }
            needle.post(order_data.server_url, order_data.server_url_data, function (err, resp, body) {
                if (err || body != Conf.bot.sender.good_answer) {
                    ...
                    return Helpers.updateRiakObject(riak_object, order_data);
                }
                //up status to STATUS_SUCCESS
                order_data.status = Conf.statuses.STATUS_SUCCESS;
                Helpers.updateRiakObject(riak_object, order_data);
            });
            if (order_data.status != Conf.statuses.STATUS_WAIT_ANSWER) {
                return false;
            }
...
// var Sender = (order_data, riak_object) => {
//
//     needle.post(order_data.server_url, order_data.server_url_data, function (err, resp, body) {
//         if (err || body != Conf.bot.sender.good_answer) {
//             Conf.log.error('Try #' + (parseInt(order_data.bot_request_count)+1) +' : Bad answer from ' + order_data.server_url);
